...
/*CONSTANTS*/
const double T_0=0.1;/*Initial time*/
const double T_F=5000;/*Final Time*/
const int TNITER=500;/*Number of iterations for the time*/
const int NITER=1000;/*Number of iterations for the energy*/
const double P2_0=1.0;
const double P2_F=2.0;
const double W0=1.0;
const double WF=2.0;
const double M0=1.0; //Constant for the f_0
...
double f_0(double tau, double w, double p2)
{
...
double Energy_0(double tau)
{
	...
	for(i=0;i<NITER;i++)
	{
		...
		for(j=0;j<NITER;j++)
		{
			...
			E+=PI*dW*dP2*f_0(tau,currW,currP2)*vv(tau,currW,currP2)/tau/tau;
		}
	}
	return E;
}
...
void calculate_f(double f[NITER][NITER],double t,double tempr)
{
	...
	for(i=0;i<NITER;i++)
	{
		...
		{
			currP2=P2_0+(double)j*dP2;
			f[i][j]=f[i][j]+(f_0(t,currW,currP2)-(f_eq(t,currW,currP2,tempr)))/t;
		}
	}
}
...
int main ()
{
	...
	calculate_f(f,T_0,Temperature);

	//Now we print the f matrix of the 1st iteration:
	//printf("         f_1 MATRIX\n\n");
	//matrix_print(NITER,f);
	//printf("\n");
	...
	for (i=0;i<=TNITER;i++)
	{
		...
		calculate_f(f,currT,Temperature);
	}
	fclose(fp);
	return 0;
}
